...
// +-------------------------------------------------+
// $Id: search_segment_search_result.class.php,v 1.28 2019-08-22 15:42:00 ccraig Exp $
	...
	protected function get_sorted_result() {
	    ...
	    switch ($aut_table) {
	        ...
	        case AUT_TABLE_AUTHPERSO:
	            $index = $table.'.authperso_index_infos_global';
	            $join = 'join '.$table.' on '.$table.'.id_authperso_authority = authorities.num_object';
	            break;
	        default :
	            $index = $table.'.index_'.$prefix;
    	        $join = 'join '.$table.' on '.$table.'.'.$prefix.'_id = authorities.num_object';
