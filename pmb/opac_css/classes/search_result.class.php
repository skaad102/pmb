...
class search_result
{
	...
	protected static function get_display_search_tabs_form()
	{
		...
		if ((!$get_query) && (!((static::$search_type == "extended_search") && ($launch_search != 1))) && (!$mode)) { //On ne met pas dans l'historique les r�sultats obtenus en cliquant sur le mot-cl� d'une notice
			rec_history();
...
// recuperamos los globales de lo autorizado en la búsqueda en la configuración del OPAC
		...
		//Ruta para autoridades personalizadas
	...
	public static function proceed_simple_search()
	{
		...
		if (static::$user_query == "") {
			if ($opac_search_other_function) {
				if (search_other_function_has_values()) {
					static::$user_query = "*";
				}
			}
		}
		if (static::$user_query != "") {
			...
			if (($nb_all_results) && ($opac_autolevel2) && !$get_query) print "<script>document.forms['search_tous'].submit();</script>";
			else print "<script>document.getElementById('search_result').style.display='';</script>";
		} else {
			print "<span class='search_no_result'>" . $msg['no_result'] . " :C </span>";
		}
	}
