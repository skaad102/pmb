...
class search_view {
	...
	public static function get_display_search() {
		...
		switch (static::$search_type) {
			//Elementos para una búsqueda simple
...
// 				$display_search .= static::get_display_extended_search_form();
	...
	public static function get_options_status_field() {
		...
		// r�cup�ration des statuts de documents utilis�s.
	...
	public static function do_ou_chercher_hidden() {
		...
		// on r�cup�re les globales de ce qui est autoris� en recherche dans le param�trage de l'OPAC
		global	$opac_modules_search_title,
		$opac_modules_search_author,
		$opac_modules_search_publisher,
		$opac_modules_search_titre_uniforme,
		...
		switch (static::$search_type) {
			...
			case "extended_search":
				...
				if ($mode_aff) {
					...
					//g�n�rer les crit�res de la multi_crit�res
					//Attention ! si on est d�j� dans une facette !
					if ($facette) {
				...
				if($onglet_persopac){
					...
					if (empty($search) && ($_GET['onglet_persopac'] || $_SERVER['REQUEST_METHOD'] == "GET")) {
						//On ne charge les champs de la pr�d�finie que si l'on vient de cliquer sur le lien
						//EDIT 13/12/17 - AR : ou si on y acc�de pas via un formulaire (utilisation du param�tres first_page_params)
			...
			case "external_search":
				...
				if ($_SESSION["ext_type"]=="multi") {
					...
					global $mode_aff;
					//Si il y a une mode d'affichage demand�, on construit l'�cran correspondant
					if ($mode_aff) {
