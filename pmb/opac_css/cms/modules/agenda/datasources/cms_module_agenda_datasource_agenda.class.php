...
// +-------------------------------------------------+
// $Id: cms_module_agenda_datasource_agenda.class.php,v 1.14 2019-07-24 14:15:50 dgoron Exp $
								...
								if(is_array($articles)){
									foreach($articles as $article){
										...
										if(!empty($event['event_start']) && !empty($event['event_end'])) {
										    if($event['event_start']['time']>=$time && $event['event_end']['time']<=$time) {
										        $events[] = $event;
										    }
										} elseif (!empty($event['event_start'])) {
                                            if($event['event_start']['time']==$time || (!empty($event['event_end']) && $event['event_end']['time']==$time)) {
                                                $events[] = $event;
                                            }
										}
									}
