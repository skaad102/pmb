...
// +-------------------------------------------------+
// $Id: search.class.php,v 1.384 2019-08-29 12:39:26 moble Exp $
...
class search {
	...
	protected function parse_search_file() {
		...
		if(!$dejaParse){
			...
			$nb_fixed_fields = 0;
			if (isset($param["FIXEDFIELDS"][0]["FIELD"]) && is_array($param["FIXEDFIELDS"][0]["FIELD"])) {
    			$nb_fixed_fields = count($param["FIXEDFIELDS"][0]["FIELD"]);
			}
			for ($i = 0; $i < $nb_fixed_fields; $i++) {
				$t=array();
	...
	protected function get_completion_authority_field($i,$n,$search, $v, $params=array()) {
		...
		if ($params['selector'] != 'instruments') {
    		$r.= "<input class='bouton' value='...' id='$fnamesans"."_authority_selector' title='".htmlentities($msg['title_select_from_list'],ENT_QUOTES,$charset)."' onclick=\"openPopUp('./select.php?what=$selector&caller=search_form&mode=un&authperso_id=$authperso_id&$p1=".$fname_id."_0&$p2=".$fnamesanslib."_0&deb_rech=&callback=authoritySelected&infield=".$fnamesans."_0', 'select_author0', 400, 400, -2, -2, 'scrollbars=yes, toolbar=no, dependent=yes, resizable=yes')\" type=\"button\">";
...
// 			}
	...
	protected function is_empty($field, $field_name) {
		if ((empty($field))||((count($field)==1)&&((string)$field[0]==""))) {
			return true;
		}
		if(count($field) > 1) {
			if((string)$field[0]=="") {
	...
	public function show_results_fichier($url,$url_to_search_form,$hidden_form=true,$search_target="", $acces=false) {
		...
		$page = (int) $page;
		$nb_per_page_search = (int) $nb_per_page_search;
		$start_page = $nb_per_page_search * $page;
