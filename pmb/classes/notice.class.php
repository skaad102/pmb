...
// +-------------------------------------------------+
// $Id: notice.class.php,v 1.317 2019-08-14 14:32:51 tsamson Exp $
...
//	require_once("$class_path/tu_notice.class.php");
	...
	class notice {
		...
		public $is_new=0; // nouveaut�
		public $date_is_new="0000-00-00 00:00:00"; // date nouveaut�
		public $create_date="0000-00-00 00:00:00"; // date cr�ation
		...
		/**
		 * Affichage des �l�ments contenu dans les onglets
		...
		public function fetch_data() {
			...
			if($result) {
				...
				//La notice est une notice num�rique ? 
				...
				// Autoriser la demande d'abonnement � l'OPAC
...
// 				if ($this->notice_mere_id || $this->duplicate_from_id) $link_annul = "<input type='button' class='bouton' value=\"{$msg[76]}\" onClick=\"unload_off();history.go(-1);\" />";
		...
		public function set_properties_from_form() {
			...
			//Ajout d'un test sur la pr�cense d'un auteur
		...
		public function save() {
			...
			if($this->id) {
				...
				if (pmb_mysql_num_rows($res_new)) {
					if($r=pmb_mysql_fetch_object($res_new)){
						if($r->notice_is_new==$this->is_new){ // pas de changement du flag
							$req_notice_date_is_new= "";
						}elseif($this->is_new){ // Changement du flag et affect� comme new
				...
				if($this->is_new){ // flag affect� comme new en cr�ation
					$req_notice_date_is_new= ", notice_date_is_new =now() ";
				}
			}
			...
			$titre_uniforme = static::getAutomaticTu($this->id);//ATTENTION si on r�cup�re le titre uniforme ici alors il est bien ajout� � la notice mais pas affich�
		...
		public function replace($by,$supp_notice=true) {
			...
			// Mise � jour des vedettes compos�es contenant cette notice
		...
		public static function del_notice ($id) {
			...
			// Suppression des r�sas planifi�es
		...
		/**
		 * Retourne les identifiants de concepts associ�s � la notice 
		 */
		...
		public function get_detail() {
			...
			switch ($this->biblio_level) {
				...
				case 's' :
				case 'a' :
					// on a affaire � un p�riodique ou � un article
