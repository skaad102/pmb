...
// +-------------------------------------------------+
// $Id: facette_search_opac.class.php,v 1.39 2019-08-01 13:16:35 btafforeau Exp $
		...
		while($row = pmb_mysql_fetch_object($result)){
			...
			$lst .= "
				<tr class='".$pair_impair."'>
					<td>
						<img src='".get_url_icon('bottom-arrow.png')."' title='".htmlentities($msg['move_bottom_arrow'], ENT_QUOTES, $charset)."' alt='".htmlentities($msg['move_bottom_arrow'], ENT_QUOTES, $charset)."' onClick=\"document.location='".static::format_url("&action=down&id=".$row->id_facette)."'\" style='cursor:pointer;'/></a>
						<img src='".get_url_icon('top-arrow.png')."' title='".htmlentities($msg['move_top_arrow'], ENT_QUOTES, $charset)."' alt='".htmlentities($msg['move_top_arrow'], ENT_QUOTES, $charset)."' onClick=\"document.location='".static::format_url("&action=up&id=".$row->id_facette)."'\" style='cursor:pointer;'/>
					</td>
					...
					$lst .= "<td ".$td_javascript.">".(count($array_subfields)>1 ? htmlentities($array_subfields[$row->facette_ss_critere],ENT_QUOTES,$charset) : $msg["admin_opac_facette_ss_critere"])."</td>
