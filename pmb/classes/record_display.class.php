...
// +-------------------------------------------------+
// $Id: record_display.class.php,v 1.41 2019-08-01 13:16:35 btafforeau Exp $
...
class record_display {
	...
	public $print_mode = 0;				// 0 affichage normal
										...
										// 4 affichage email : sans lien sauf url associ�e
...
// 	public $drag = 0;
	...
	protected function get_display_pperso() {
		...
		if (!$this->p_perso->no_special_fields) {
			...
			for ($i=0; $i<count($perso_["FIELDS"]); $i++) {
				$p=$perso_["FIELDS"][$i];
				// ajout de && ($p['OPAC_SHOW']||$this->show_opac_hidden_fields) afin de masquer les champs masqu� de l'OPAC en diff de bannette.
	...
	public function show_expl_per_notice($no_notice, $link_expl='',$expl_bulletin=0 ) {
		...
		// $link_expl= lien associ� � l'exemplaire avec !!expl_id!! et !!expl_cb!! � mettre � jour
		...
		// r�cup�ration du nombre total d'exemplaires
		if($expl_bulletin){
					...
					for ($i=0; $i<count($colonnesarray); $i++) {
						...
						if (substr($colonnesarray[$i],0,1)=="#") {
							...
							if($expl->pret_retour) {
								...
								if ($reserve) $situation = "<strong>".$msg['expl_reserve']."</strong>"; // exemplaire r�serv�
								elseif($expl->expl_retloc) $situation = $msg['resa_menu_a_traiter'];  // exemplaire � traiter
								elseif ($aranger) $situation = "<strong>".$msg['resa_menu_a_ranger']."</strong>"; // exemplaire � ranger
