...
// +-------------------------------------------------+
// $Id: fields_empr.inc.php,v 1.228 2019-09-03 14:01:35 dgoron Exp $
...
function aff_query_auth_empr($field,&$check_scripts,$script="") {
	...
	for ($i=0; $i<$n; $i++) {
		...
		if ($i == $n-1 && $options['MULTIPLE'][0]['value']=="yes") $display_temp.="<input id='button_add_".$field['NAME']."' type='button' class='bouton' value='+' onClick=\"add_".$field["NAME"]."();\"/>";
		if ($options['MULTIPLE'][0]['value']=="yes") {
